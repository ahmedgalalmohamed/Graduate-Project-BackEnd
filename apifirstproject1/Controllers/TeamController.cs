...
namespace Graduate_Project_BackEnd.Controllers
{
    ...
    public class TeamController : Controller
    {
        ...
        public IActionResult getMyTeam([FromForm] int id)
        {
            ...
            var leader = DB.Courses_Students.Where(cs => cs.TeamID == id && cs.Team.LeaderID == cs.StudentID).Include(t => t.Team).Include(cs => cs.Student).Select(s => new { s.StudentID, s.Student.Name, s.Student.Email, teamId = s.TeamID ,course = s.Course.Name,team = s.Team.Name});
